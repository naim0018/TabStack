  ...
  if (mode === "analog") {
    ...
    return (
      ...
      >
        ...
        {/* Minute Hand */}
        <div
          className="absolute w-[4px] h-[35%] bg-text-secondary rounded-full origin-bottom z-20 transition-transform duration-500 ease-in-out"
          style={{
            bottom: "50%",
            left: "calc(50% - 2px)",
        ...
        {/* Center Dot */}
        <div
          className="absolute w-3 h-3 bg-accent rounded-full z-40 border-2 border-bg-card shadow-lg"
        ...
        {[...Array(12)].map((_, i) => (
          ...
          >
            <div
              className={`w-0.5 ${
                i % 3 === 0 ? "h-3 bg-accent" : "h-1.5 bg-border-card"
              } mx-auto`}
            />
          </div>
        ))}
    ...
    <div className="glass mx-auto p-4 bg-bg-card border border-border-card rounded-2xl shadow-sm">
      ...
      <div className="grid grid-cols-7 gap-1 text-center mb-2">
        {dayNames.map((d) => (
          <div
            key={d}
            className="text-[11px] font-bold text-text-secondary opacity-40"
          >
            {d}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((day, idx) => (
          <div
            key={idx}
            className={`
              aspect-square flex items-center justify-center text-[12px] rounded-lg transition-all
              ${
                day === today && isCurrentMonth
                  ? "bg-accent text-black scale-110 text-[14px] font-semibold"
                  : "text-text-primary/70 hover:bg-border-card"
              }
